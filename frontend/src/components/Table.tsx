        ...
        <table className="w-full min-w-[640px]">
          <thead>
            <tr className="bg-white/[0.02]">
              {columns.map((col) => (
                <th key={col.key} className={`px-3.5 py-2.5 text-left text-xs font-semibold text-muted uppercase tracking-wide ${col.className ?? ""}`}>
                  {col.header}
                </th>
              ))}
            </tr>
      ...
      {pagination && pagination.totalPages > 0 && (
        <div className="flex items-center justify-between px-3.5 py-2.5 border-t border-line bg-white/[0.02] text-sm text-muted flex-wrap gap-2">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button className="btn-ghost !px-2.5 !py-1 !text-sm !min-h-0" disabled={pagination.page <= 1} onClick={() => onPageChange?.(pagination.page - 1)}>上一页</button>
            <button className="btn-ghost !px-2.5 !py-1 !text-sm !min-h-0" disabled={pagination.page >= pagination.totalPages} onClick={() => onPageChange?.(pagination.page + 1)}>下一页</button>
          </div>
        </div>
      )}
    </div>
